...
PACKAGE_OPENWRT_6XY=("r66s" "r68s" "e25" "photonicat" "cm3")
            ...
            for kernel_var in "${build_kernel[@]}"; do
                {
                    ...
                    [[ -n "$(echo "${PACKAGE_OPENWRT_6XY[@]}" | grep -w "${PACKAGE_VAR}")" && "${kernel_var:0:2}" != "6." ]] && {
                        echo -e "${STEPS} (${i}.${k}) ${NOTE} Based on the settings in <PACKAGE_OPENWRT_6XY>, skip the [ ${PACKAGE_VAR} - ${vb}/${kernel_var} ] build."
                        let k++
                        continue
                    }
